...
let rec eval (s: string) (tl: term list): string =

  match tl with
  | (Name a) :: tl -> (eval (s^"Push "^a^"\nLookup\n") tl)
 
  | (Fun (f, arg, t)) :: tl -> (
      eval (s^"Fun "^f^" "^arg^"\n"^(eval "" [t])^"End\nPush "^f^"\nLookup\n") tl)
 
  | (App (t1, t2)) :: tl -> (
      eval (s^(eval "" [t1])^(eval "" [t2])^"Call\n") tl)
  ...
  | (Int i) :: tl -> 
      eval (s^"Push "^(string_of_int(i)^"\n")) tl

  | (Add (v1,v2)) :: tl -> (
      eval (s^(eval "" [v1])^(eval "" [v2])^"Add\n") tl)

  | (Sub (v1,v2)) :: tl -> (
      eval (s^(eval "" [v1])^(eval "" [v2])^"Sub\n") tl)

  | (Mul (v1, v2)) :: tl -> (
      eval (s^(eval "" [v1])^(eval "" [v2])^"Mul\n") tl)

  | (Div (v1, v2)) :: tl -> (
      eval (s^(eval "" [v1])^(eval "" [v2])^"Div\n") tl)
